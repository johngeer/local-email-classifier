...
This is a map of how `email_classifier` is put together and where each
responsibility lives. It points at the code and states the invariants that hold across it, 
leaving fine detail to the code itself. 
...
**Core** (`src/core/mod.rs`) is pure, and takes the embedding as an argument so the
...
**Shell** (`src/shell/mod.rs`) is all IO behind three functions:
...
Features are bounded by construction, so there is no normalization step and no
stored stats: the embedding is unit-norm (each of its 384 components is signed,
in ≈[-1,1]), and the eight history features are in [0,1]. Each history block is
three Dirichlet-smoothed class
...
- chronological-replay training (removes the v1 training-time count leak)
- `confidence · P(pₙ)` history interaction terms
- batched embedding for `train`/`eval`
- proportional-odds ordinal regression
- a Tranco-rank feature
```
